...
#[test]
fn ger_supports_unit_and_basic_scalar_returns_with_exact_replacements() {
    let source = r#"
fn unit_fn() {
    ping();
}
...
"#;
    ...
    assert!(
        ...
        "expected `ping();` -> `return;` for unit function: {ger_mutants:?}"
    );
    assert!(
        ger_mutants
            .iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return false;"),
        "expected `ping();` -> `return false;` for bool function: {ger_mutants:?}"
    );
    assert!(
        ger_mutants
            .iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return 0;"),
        "expected `ping();` -> `return 0;` for int function: {ger_mutants:?}"
    );
    assert!(
        ger_mutants
            .iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return 0.0;"),
        "expected `ping();` -> `return 0.0;` for float function: {ger_mutants:?}"
    );
}

#[test]
fn ger_skips_unsupported_rust_return_types() {
    let source = r#"
fn string_fn() -> String {
    let x = 1;
    format!("{x}")
...
"#;
    ...
    let mutants = RustLanguageEngine::new().mutate(&target);
    assert!(
        !mutants.iter().any(|m| m.mutation_slug == "GER"),
        "GER should skip unsupported Rust return types"
    );
}

#[test]
fn ger_does_not_target_return_statements_or_closure_body_statements_in_rust() {
    let source = r#"
fn outer() -> bool {
    let f = || {
        ping();
...
"#;
    ...
    assert!(
        ger_mutants.iter().any(
            |m| m.old_text.trim_start().starts_with("if true") && m.new_text == "return false;"
        ),
        "expected outer if-expression to be mutated to an early return: {ger_mutants:?}"
    );
    assert!(
        !ger_mutants
            .iter()
            .any(|m| m.old_text.contains("return f();")),
        "GER should not target existing return statements: {ger_mutants:?}"
    );
    assert!(
        !ger_mutants
            .iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return false;"),
        "GER should not mutate statements inside closures: {ger_mutants:?}"
    );
}

#[test]
fn ger_skips_result_and_option_return_types() {
    let source = r#"
...
#[test]
fn ger_skips_str_ref_return_type() {
    let source = r#"
...
#[test]
fn ger_picks_correct_return_type_across_multiple_functions() {
    let source = r#"
fn bool_fn() -> bool {
    ping();
    true
...
"#;
    ...
    // Both ping() calls should have different replacements
    assert!(
        ger.iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return false;"),
        "bool_fn's ping() should become return false: {ger:?}"
    );
    assert!(
        ger.iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return 0;"),
        "int_fn's ping() should become return 0: {ger:?}"
    );
}
...
"#;
    ...
    assert!(
        ger.iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return 0;"),
        "GER should replace statement before return, making original return dead code: {ger:?}"
    );
}
