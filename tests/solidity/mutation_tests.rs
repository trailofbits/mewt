...
#[test]
fn ger_supports_basic_solidity_returns_with_exact_replacements() {
    let source = r#"
pragma solidity ^0.8.0;
...
"#;
    ...
    assert!(
        ...
        "expected `ping();` -> `return;` for no-return Solidity functions: {ger_mutants:?}"
    );
    assert!(
        ger_mutants
            .iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return 0;"),
        "expected `ping();` -> `return 0;` for scalar Solidity returns: {ger_mutants:?}"
    );
    assert!(
        ger_mutants
            .iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return (0, false);"),
        "expected `ping();` -> `return (0, false);` for tuple Solidity returns: {ger_mutants:?}"
    );
}

#[test]
fn ger_skips_unsupported_solidity_return_types() {
    let source = r#"
pragma solidity ^0.8.0;
...
"#;
    ...
    assert!(
        ...
        "GER should skip unsupported Solidity return types"
    );
}

#[test]
fn ger_does_not_target_existing_return_statements_in_solidity() {
    let source = r#"
pragma solidity ^0.8.0;
...
"#;
    ...
    assert!(
        !mutants
            .iter()
            .any(|m| m.mutation_slug == "GER" && m.old_text.trim_start().starts_with("return ")),
        "GER should not target existing Solidity return statements"
    );
}
...
"#;
    ...
    // The if-statement itself should get GER, and ping() inside it should NOT
    // (it's inside a nested block, the ancestor check prevents duplication)
    assert!(
        !ger.is_empty(),
        "GER should fire on statements inside function body: {ger:?}"
    );
}
...
"#;
    ...
    assert!(
        ...
        "getBool's ping() should become return false: {ger:?}"
    );
    assert!(
        ger.iter()
            .any(|m| m.old_text.trim() == "ping();" && m.new_text == "return 0;"),
        "getUint's ping() should become return 0: {ger:?}"
    );
}
